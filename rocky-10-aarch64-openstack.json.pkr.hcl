...
source "qemu" "rocky-10" {
  ...
  boot_command     = [
    ...
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rocky-10/ks-aarch64.cfg",
  ...
  headless          = true
  http_directory    = "http"
  iso_checksum      = "file:${var.mirror}/pub/rocky/10/isos/aarch64/CHECKSUM"
  iso_url           = "${var.mirror}/pub/rocky/10/isos/aarch64/Rocky-10-latest-aarch64-minimal.iso"
