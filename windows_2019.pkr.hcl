...
locals {
  iso_target_path = "${path.root}/builds/iso/windows-2019-${substr(sha256(var.iso_url), 0, 8)}.iso"
}

source "qemu" "windows_2019" {
  accelerator       = "kvm"
  boot_wait         = "10s"
  cpus              = 4
  cpu_model         = "host"
  ...
  memory            = 8192
  output_directory  = "output-windows_2019"
...
build {
  sources = ["source.qemu.windows_2019"]
