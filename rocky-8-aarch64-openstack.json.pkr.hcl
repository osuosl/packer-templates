...
variable "mirror" {
  type    = string
  default = "https://download.rockylinux.org"
}
...
source "qemu" "rocky-8" {
  ...
  boot_command     = [
    ...
    "inst.stage2=hd:LABEL=AlmaLinux-8-10-aarch64-dvd ",
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rocky-8/ks-aarch64.cfg<enter>",
  ...
  headless         = true
  http_directory   = "http"
  iso_checksum     = "file:${var.mirror}/pub/rocky/8/isos/aarch64/CHECKSUM"
  iso_url          = "${var.mirror}/pub/rocky/8/isos/aarch64/Rocky-8-latest-aarch64-minimal.iso"
