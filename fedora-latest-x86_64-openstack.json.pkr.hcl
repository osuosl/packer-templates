...
source "qemu" "fedora-latest" {
  ...
  boot_command      = [
    ...
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/fedora-latest/ks-x86_64.cfg",
  ...
  headless          = true
  http_directory    = "http"
  iso_checksum      = "file:${var.mirror}/releases/${var.release}/Everything/x86_64/iso/Fedora-Everything-${var.release}-${var.compose}-x86_64-CHECKSUM"
  iso_url           = "${var.mirror}/releases/${var.release}/Everything/x86_64/iso/Fedora-Everything-netinst-x86_64-${var.release}-${var.compose}.iso"
  memory            = 2048
