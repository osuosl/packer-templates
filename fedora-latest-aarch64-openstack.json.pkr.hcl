...
variable "mirror" {
  type    = string
  default = "https://fedora.osuosl.org/linux"
}
...
source "qemu" "fedora-latest" {
  ...
  boot_command     = [
    ...
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/fedora-latest/ks-aarch64.cfg",
  ...
  headless          = true
  http_directory    = "http"
  iso_checksum      = "file:${var.mirror}/releases/${var.release}/Everything/aarch64/iso/Fedora-Everything-${var.release}-${var.compose}-aarch64-CHECKSUM"
  iso_url           = "${var.mirror}/releases/${var.release}/Everything/aarch64/iso/Fedora-Everything-netinst-aarch64-${var.release}-${var.compose}.iso"
