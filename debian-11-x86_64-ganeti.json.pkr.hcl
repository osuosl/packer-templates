...
source "qemu" "debian-11-ganeti" {
  ...
  boot_command     = [
      ...
      " preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian-11/preseed-ganeti.cfg",
      " url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian-11/preseed-ganeti.cfg",
      "<enter>"
    ]
