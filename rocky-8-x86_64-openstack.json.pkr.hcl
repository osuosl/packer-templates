...
source "qemu" "rocky-8" {
  accelerator      = "kvm"
  boot_command     = [
    "<tab> text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rocky-8/ks-x86_64.cfg<enter><wait>"
  ]
  boot_wait        = "10s"
  ...
  headless         = true
  http_directory   = "http"
  iso_checksum     = "file:${var.mirror}/pub/rocky/8/isos/x86_64/CHECKSUM"
  iso_url          = "${var.mirror}/pub/rocky/8/isos/x86_64/Rocky-8-latest-x86_64-minimal.iso"
