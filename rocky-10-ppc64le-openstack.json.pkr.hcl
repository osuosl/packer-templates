...
source "qemu" "rocky-10" {
  ...
  boot_command     = [
    ...
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rocky-10/ks-ppc64le.cfg",
  ...
  headless          = true
  http_directory    = "http"
  iso_checksum      = "file:${var.mirror}/pub/rocky/10/isos/ppc64le/CHECKSUM"
  iso_url           = "${var.mirror}/pub/rocky/10/isos/ppc64le/Rocky-10-latest-ppc64le-minimal.iso"
