...
source "qemu" "centos-stream-10" {
  ...
  boot_command      = [
    ...
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos-stream-10/ks-x86_64.cfg",
  ...
  headless          = true
  http_directory    = "http"
  iso_checksum      = "file:https://centos-stream.osuosl.org/10-stream/BaseOS/x86_64/iso/SHA256SUM"
  iso_url           = "${var.mirror}/10-stream/BaseOS/x86_64/iso/CentOS-Stream-10-${var.release}-x86_64-boot.iso"
  memory            = 2048
