...
variable "mirror" {
  type    = string
  default = "http://centos-stream.osuosl.org"
}
...
source "qemu" "centos-stream-9" {
  accelerator      = "kvm"
  boot_command     = [
    "<up><tab> text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos-stream-9/ks-x86_64.cfg<enter><wait>"
  ]
  ...
  headless         = true
  http_directory   = "http"
  iso_checksum     = "file:${var.mirror}/9-stream/BaseOS/x86_64/iso/CentOS-Stream-9-latest-x86_64-dvd1.iso.SHA256SUM"
  iso_url          = "${var.mirror}/9-stream/BaseOS/x86_64/iso/CentOS-Stream-9-latest-x86_64-dvd1.iso"
