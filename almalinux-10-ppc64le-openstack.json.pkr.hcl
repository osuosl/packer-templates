...
source "qemu" "almalinux-10" {
  ...
  boot_command     = [
    ...
    "inst.stage2=hd:LABEL=AlmaLinux-10-0-ppc64le-dvd ",
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/almalinux-10/ks-ppc64le.cfg<enter>",
  ...
  headless         = true
  http_directory   = "http"
  iso_checksum     = "file:https://almalinux.osuosl.org/10/isos/ppc64le/CHECKSUM"
  iso_url          = "${var.mirror}/10/isos/ppc64le/AlmaLinux-10-latest-ppc64le-minimal.iso"
