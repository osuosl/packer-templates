...
source "qemu" "rocky-9" {
  ...
  boot_command      = [
    ...
    "inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rocky-9/ks-ppc64le.cfg",
  ...
  headless          = true
  http_directory    = "http"
  iso_checksum      = "file:${var.mirror}/pub/rocky/9/isos/ppc64le/CHECKSUM"
  iso_url           = "${var.mirror}/pub/rocky/9/isos/ppc64le/Rocky-9-latest-ppc64le-minimal.iso"
